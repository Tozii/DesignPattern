...
## 介绍
&emsp;&emsp;在软件系统中，有时需要创建一个复杂对象，并且这个复杂对象由其各部分子对象通过一定的步骤组合而成
...
**经典创建者模式，创建对象执行步骤采用硬编码，在此基础上使用多播委托改进**
...
{
    protected IList<Action<string>> steps=new List<Action<string>>();
...
public class Builder:BuilderBase
{
    public Builder()
    {
        steps.Add(computer.Add("CPU"));
        steps.Add(computer.Add("主板"));
    }
}
```